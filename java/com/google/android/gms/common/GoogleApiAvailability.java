...
public class GoogleApiAvailability extends zzc {
    ...
    @Nullable
    public zzaar zza(Context context, com.google.android.gms.internal.zzaar.zza zza) {
        IntentFilter intentFilter = new IntentFilter("android.intent.action.PACKAGE_ADDED");
        intentFilter.addDataScheme("package");
        BroadcastReceiver zzaar = new zzaar(zza);
    ...
    void zza(Context context, int i, String str, PendingIntent pendingIntent) {
        if (i == 18) {
            ...
            if (com.google.android.gms.common.util.zzi.zzaJ(context)) {
                zzac.zzar(zzs.zzyG());
                build = new Notification.Builder(context).setSmallIcon(context.getApplicationInfo().icon).setPriority(2).setAutoCancel(true).setContentTitle(zzh).setStyle(new BigTextStyle().bigText(zzj)).addAction(C0225R.drawable.common_full_open_on_phone, resources.getString(C0225R.string.common_open_on_phone), pendingIntent).build();
            } else {
                build = new NotificationCompat.Builder(context).setSmallIcon(17301642).setTicker(resources.getString(C0225R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setAutoCancel(true).setContentIntent(pendingIntent).setContentTitle(zzh).setContentText(zzj).setLocalOnly(true).setStyle(new NotificationCompat.BigTextStyle().bigText(zzj)).build();
            }
            switch (i) {
                case 1:
                case 2:
    ...
    public boolean zza(Activity activity, @NonNull zzaax zzaax, int i, int i2, OnCancelListener onCancelListener) {
        Dialog zza = zza((Context) activity, i, zzi.zza(zzaax, zzb(activity, i, "d"), i2), onCancelListener);
        if (zza == null) {
            return false;
        }
