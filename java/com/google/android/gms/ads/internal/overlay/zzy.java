...
public class zzy extends Thread implements OnFrameAvailableListener, zza {
    ...
    private String zzhX() {
        zzft zzft = zzfx.zzCI;
        return !((String) zzft.get()).equals(zzft.zzfm()) ? (String) zzft.get() : "attribute highp vec3 aPosition;varying vec3 pos;void main() {  gl_Position = vec4(aPosition, 1.0);  pos = aPosition;}";
    }

    private String zzhY() {
        zzft zzft = zzfx.zzCJ;
        return !((String) zzft.get()).equals(zzft.zzfm()) ? (String) zzft.get() : "#extension GL_OES_EGL_image_external : require\n#define INV_PI 0.3183\nprecision highp float;varying vec3 pos;uniform samplerExternalOES uSplr;uniform mat3 uVMat;uniform float uFOVx;uniform float uFOVy;void main() {  vec3 ray = vec3(pos.x * tan(uFOVx), pos.y * tan(uFOVy), -1);  ray = (uVMat * ray).xyz;  ray = normalize(ray);  vec2 texCrd = vec2(    0.5 + atan(ray.x, - ray.z) * INV_PI * 0.5, acos(ray.y) * INV_PI);  gl_FragColor = vec4(texture2D(uSplr, texCrd).xyz, 1.0);}";
    }
    ...
    public void run() {
        Object obj = 1;
        if (this.zzOa == null) {
            zzpy.m9e("SphericalVideoProcessor started with no output texture.");
            this.zzOf.countDown();
            return;
        }
        boolean zzhV = zzhV();
